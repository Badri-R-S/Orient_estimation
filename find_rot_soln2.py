...
def find_rotation_angle(template_img, test_img):
    ...
    #orb = cv2.ORB_create(nfeatures=5000, scaleFactor=1.2, nlevels=8, edgeThreshold=15, firstLevel=0, WTA_K=2, patchSize=31)
    ...
    if len(good_matches) < 10:
        raise RuntimeError("Not enough good matches found to estimate homography.")
    ...
    if H is not None:
        ...
        #rotation_angle = np.arctan2(H[0, 1], H[0, 0]) * 180 / np.pi
...
if __name__ == "__main__":
    ...
    for template_filename in sorted(os.listdir(template_folder)):
        if template_filename.startswith("template_masked"):
